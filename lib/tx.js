...
class ShieldedTx {
    static async fromData(txData, txType, acc, snarkParams, web3, worker) {
        ...
        const txValid = libzkbob_rs_wasm_web_1.Proof.verify(snarkParams.transferVk, txProof.inputs, txProof.proof);
        if (!txValid) {
            throw new Error('invalid tx proof');
        }
        const treeValid = libzkbob_rs_wasm_web_1.Proof.verify(snarkParams.treeVk, treeProof.inputs, treeProof.proof);
        if (!treeValid) {
