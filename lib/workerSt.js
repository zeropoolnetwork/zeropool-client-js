...
const obj = {
    async initWasm(url, paramUrls) {
        ...
        await (0, libzeropool_rs_wasm_web_1.default)(url);
        const cache = await file_cache_1.FileCache.init();
        const txParamsData = await cache.getOrCache(paramUrls.txParams);
        txParams = libzeropool_rs_wasm_web_1.Params.fromBinary(new Uint8Array(txParamsData));
        const treeParamsData = await cache.getOrCache(paramUrls.treeParams);
        treeParams = libzeropool_rs_wasm_web_1.Params.fromBinary(new Uint8Array(treeParamsData));
        console.info('Web worker init complete.');
    },
    async proveTx(pub, sec) {
        return new Promise(async (resolve) => {
            console.debug('Web worker: proveTx');
            const result = libzeropool_rs_wasm_web_1.Proof.tx(txParams, pub, sec);
            resolve(result);
        });
    },
    async proveTree(pub, sec) {
        return new Promise(async (resolve) => {
            ...
            const result = libzeropool_rs_wasm_web_1.Proof.tree(treeParams, pub, sec);
            resolve(result);
        });
    },
};
(0, comlink_1.expose)(obj);
//# sourceMappingURL=workerSt.js.map