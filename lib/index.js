...
const libzkbob_rs_wasm_web_1 = __importStar(require("libzkbob-rs-wasm-web"));
...
async function init(wasmPath, workerPath, snarkParams) {
    ...
    await (0, libzkbob_rs_wasm_web_1.default)(wasmPath);
    const txParamsData = await fileCache.getOrCache(snarkParams.transferParamsUrl);
    const transferParams = libzkbob_rs_wasm_web_1.Params.fromBinary(new Uint8Array(txParamsData));
    const treeParamsData = await fileCache.getOrCache(snarkParams.treeParamsUrl);
    const treeParams = libzkbob_rs_wasm_web_1.Params.fromBinary(new Uint8Array(treeParamsData));
