...
async function init(wasmPath, workerPath, snarkParams) {
    ...
    const transferParams = libzeropool_rs_wasm_web_1.Params.fromBinary(new Uint8Array(txParamsData), true, true);
    const treeParamsData = await fileCache.getOrCache(snarkParams.treeParamsUrl);
    const treeParams = libzeropool_rs_wasm_web_1.Params.fromBinary(new Uint8Array(treeParamsData), true, true);
