...
export function verifyShieldedAddress(address) {
...
export function ethAddrToBuf(address) {
...
export function hexToBuf(hex, bytesCnt = 0) {
    if (hex.length % 2 !== 0) {
...
export function addressFromSignature(signature, signedData) {
    let sigFields = util.fromRpcSig(addHexPrefix(signature));
    const dataBuf = hexToBuf(signedData);
    const prefix = Buffer.from("\x19Ethereum Signed Message:\n");
    const prefixedSignedData = util.keccak(Buffer.concat([prefix, Buffer.from(String(dataBuf.length)), dataBuf]));
