...
export class ZeropoolClient {
     ...
     */
    async deposit(tokenAddress, amountWei, sign, fromAddress = null, fee = '0', isBridge = false, outsWei = []) {
    ...
    getLatestMinedIndex(tokenAddress) {
        return Number(localStorage.getItem(`zp.${tokenAddress}.latestMinedIndex`) || '0');
    }
    setLatestMinedIndex(tokenAddress, index) {
