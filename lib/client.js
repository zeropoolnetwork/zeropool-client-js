...
export class ZeropoolClient {
    constructor() {
        this.transfer = this.transferSingle;
        this.withdraw = this.withdrawSingle;
    }
    ...
    async getBalances(tokenAddress, updateState = true) {
        if (updateState) {
            await this.updateState(tokenAddress);
        }
    ...
    async getOptimisticTotalBalance(tokenAddress, updateState = true) {
    ...
    // Get history records
    async getAllHistory(tokenAddress, updateState = true) {
        if (updateState) {
            await this.updateState(tokenAddress);
        }
    ...
    async waitJobsCompleted(tokenAddress, jobIds) {
        const token = this.tokens[tokenAddress];
        let promises = jobIds.map(async (jobId) => {
            const txHashes = await this.waitJobCompleted(tokenAddress, jobId);
    ...
    async waitJobQueued(tokenAddress, jobId) {
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const INTERVAL_MS = 1000;
        let hashes;
    ...
    async depositPermittableV2(tokenAddress, amountWei, signTypedData, fromAddress = null, feeWei = BigInt(0), outputs = []) {
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
    ...
    // // Returns jobId from the relayer or throw an Error
    async transferMulti(tokenAddress, to, amountWei, feeWei = BigInt(0)) {
        const state = this.zpStates[tokenAddress];
        const token = this.tokens[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
        ...
        for (let index = 0; index < txParts.length; index++) {
            ...
            const startProofDate = Date.now();
            const txProof = await this.worker.proveTx(oneTxData.public, oneTxData.secret);
            const proofTime = (Date.now() - startProofDate) / 1000;
            console.log(`Proof calculation took ${proofTime.toFixed(1)} sec`);
            const txValid = zp.Proof.verify(this.snarkParams.transferVk, txProof.inputs, txProof.proof);
            if (!txValid) {
    ...
    // Returns jobId from the relayer or throw an Error
    async withdrawMulti(tokenAddress, address, amountWei, feeWei = BigInt(0)) {
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
        if (amountWei < MIN_TX_AMOUNT) {
    ...
    // Returns jobId
    async deposit(tokenAddress, amountWei, sign, fromAddress = null, // this field is only for substrate-based network,
    // it should be null for EVM
    feeWei = BigInt(0), outputs = []) {
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
        if (amountWei < MIN_TX_AMOUNT) {
    ...
    async transferSingle(tokenAddress, outsWei, feeWei = BigInt(0)) {
        await this.updateState(tokenAddress);
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
    ...
    async withdrawSingle(tokenAddress, address, amountWei, feeWei = BigInt(0)) {
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
        const denominator = this.getDenominator(tokenAddress);
    ...
    async atomicTxFee(tokenAddress) {
    ...
    async feeEstimate(tokenAddress, amountWei, txType, updateState = true) {
        const relayer = await this.getRelayerFee(tokenAddress);
        const l1 = BigInt(0);
        let txCnt = 1;
    ...
    async getRelayerFee(tokenAddress) {
    ...
    async minTxAmount(tokenAddress) {
        return MIN_TX_AMOUNT;
    }
    // Account + notes balance excluding fee needed to transfer or withdraw it
    async calcMaxAvailableTransfer(tokenAddress, updateState = true) {
        const state = this.zpStates[tokenAddress];
        if (updateState) {
            await this.updateState(tokenAddress);
        }
    ...
    async getTransactionParts(tokenAddress, amountWei, feeWei, updateState = true) {
        const state = this.zpStates[tokenAddress];
        if (updateState) {
            await this.updateState(tokenAddress);
        }
    ...
    async getLimits(tokenAddress, address = undefined, directRequest = false) {
        const token = this.tokens[tokenAddress];
        async function fetchLimitsFromContract(network) {
        ...
        function defaultLimits() {
            ...
            return {
                deposit: {
                    singleOperation: BigInt(10000000000000),
                    daylyForAddress: {
                        total: BigInt(10000000000000),
                        available: BigInt(10000000000000), // 10k tokens
                    },
                    daylyForAll: {
                        total: BigInt(100000000000000),
    ...
    async updateStateOptimisticWorker(tokenAddress) {
        ...
        if (optimisticIndex > startIndex) {
            ...
            ;
            let totalState = new Map();
            let initRes = { txCount: 0, maxMinedIndex: -1, maxPendingIndex: -1, state: totalState };
    ...
    async getNewState(tokenAddress) {
        const OUTPLUSONE = CONSTANTS.OUT + 1;
        const zpState = this.zpStates[tokenAddress];
        const token = this.tokens[tokenAddress];
        const state = this.zpStates[tokenAddress];
    ...
    // -------------------------------------------------------------------------------
    async fetchTransactionsOptimistic(relayerUrl, offset, limit = 100) {
    ...
    async sendTransactions(relayerUrl, txs) {
    ...
    async limits(relayerUrl, address) {
    ...
    async fetchTransactions(relayerUrl, offset, limit = 100) {
