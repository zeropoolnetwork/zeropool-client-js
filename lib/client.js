...
class ZeropoolClient {
    ...
    async updateState(tokenAddress) {
        if (this.updateStatePromise == undefined) {
            this.updateStatePromise = this.updateStateWorker(tokenAddress).then(response => {
                this.updateStatePromise = undefined;
            });
        }
        else {
            console.info(`The state currentlly updating, waiting for finish...`);
        }
            ...
            do {
                ...
                for (let i = 0; i < txs.length; ++i) {
                    ...
                    if (myMemo) {
                        ...
                        let hist = (0, history_1.convertToHistory)(myMemo, txHash, rpc);
    ...
    cacheShieldedTx(tokenAddress, ciphertext, hashes, index) {
        ...
        if (pair) {
            ...
            const in_notes = pair.notes.reduce((acc, note, noteIndex) => {
            ...
            const out_notes = pair.notes.reduce((acc, note, noteIndex) => {
                const address = (0, libzeropool_rs_wasm_web_1.assembleAddress)(note.d, note.p_d);
