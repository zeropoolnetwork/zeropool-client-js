...
class HistoryStorage {
    ...
    async getAllHistory(rpc) {
        ...
        let allRecords = await this.db.getAll(TX_TABLE);
        return allRecords;
    }
    async syncHistory(rpc) {
        const startTime = Date.now();
        let syncIndex = await this.db.get(HISTORY_STATE_TABLE, 'sync_index');
        if (syncIndex == undefined) {
            syncIndex = -1;
        }
        let allUnprocessedMemos = await this.db.getAll(DECRYPTED_MEMO_TABLE, IDBKeyRange.lowerBound(syncIndex, true));
    ...
    async convertToHistory(memo, rpcUrl) {
